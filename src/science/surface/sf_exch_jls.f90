...
open(unit=1254,file='sf_exch_mp',status="unknown", &
action="write", form="formatted",position='append' )
   WRITE(1254,*) "cable% mp% row_length ", cable% mp% row_length
   WRITE(1254,*) "cable% mp% rows ", cable% mp% rows
   write(1254,*) " LAND_FIELD ",  cable% mp% LAND_pts   
   write(1254,*) " ntiles ",  cable% mp% ntiles
   write(1254,*) " npfts ",  cable% mp% npft  
   write(1254,*) " sm_levels ",  cable% mp% sm_levels
   write(1254,*) " TIMESTEP_NUMBER ", cable% mp% TIMESTEP_width 
close(1254)


open(unit=1253,file='exch_cable_lat',status="unknown", &
action="write", form="formatted",position='append' )
do i=1, cable% mp% row_length
   do j=1,cable% mp% rows
      WRITE(1253,*),i,j, ( cable% mp% latitude(i,j) )
   enddo
enddo
close(1253)
...
! DEPENDS ON: cable_explicit_driver
  CALL cable_explicit_driver(                                       &
             ...
             cable% mp% timestep_number, cable% mp% mype )
